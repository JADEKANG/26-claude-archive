...
리서치 브리프 공통 템플릿: `~/.claude/eo-thinking-mode/research-template.md` (Section 3은 셜록작가 버전 적용)
...
### 레이어 0 — 적극 선발 기준 (우선순위 가이드)

> 필터가 아니라 **끌어올리는 기준**이다. 4개 모두 충족할 필요는 없다.
> 하지만 더 많이 부합할수록 좋은 후보다. 레이어 A를 통과한 후보들 중 우선순위를 매길 때 활용한다.

| 기준 | 설명 | 신호 예시 |
|---|---|---|
| **① 주장을 살고 있는 사람** | 책 쓰고 강연하는 게 주업이 아니라, 연구하거나 만들다가 부산물로 책이 나온 사람. "책이 커리어의 목적지"가 아니라 "Frontier 행보의 부산물"로 책이 나온 사람. | 현직 연구자/창업자/엔지니어. 책 1권만 있고 다음 행보가 있다. |
| **② 본인이 명명한 개념이 있는 사람** | 구체적인 이름이 붙은 자기 개념이 있는 사람. 막연한 프레임워크나 조언 묶음이 아닌, "Flash Teams", "AI Engineering"처럼 본인이 정의한 개념. 개념이 있어야 인터뷰에서 Intellectual Tension이 생긴다. | 책 제목 = 본인의 개념명. 해당 개념이 학계/업계에서 인용되기 시작함. |
| **③ TTM 빌더 시청자가 자기 얘기로 듣는 사람** | C-suite 리더십, 조직 변화 관리, 명상 기반 리더십 → 빌더(22-32세)한테는 "남 얘기". 빌더가 "나 당장 이거 적용해볼 수 있겠는데"라고 느끼는 주제여야 한다. | 주제가 일하는 방식/학습/커리어/빌딩 직접 관련. 독자층이 실무자/엔지니어/연구자. |
| **④ 주장에 증거가 있는 사람** | 사변이나 직관이 아닌 — 실험 데이터, 실제 빌딩 경험, 피어 리뷰. 주장이 뾰족하면서 근거도 있어야 "이 사람 틀릴 수도 있다"는 긴장감이 생기고 인터뷰가 살아있다. | MIT Press / O'Reilly 등 동료 검토 출판사. 논문 인용 수. 실제 프로덕트/실험 결과 있음. |

**레이어 0 판정 방식**: 4개 중 해당 개수를 표시한다 (예: `레이어 0: 3/4`). 2개 미만이면 레이어 A Pass여도 보류 우선 고려.

---

### 레이어 A — 선발 필터 (7개 기준 Pass/Fail)

각 후보를 아래 7개 기준으로 판정한다. **Fail이 2개 이상이면 제외. AI 옵티미즘 Fail은 단독으로도 제외 사유.**

| 기준 | Pass | Fail |
|---|---|---|
| **AI 옵티미즘 정합** | "AI를 어떻게 잘 써서 프론티어가 될 것인가" 방향. AI를 기회/도구/가능성으로 보는 스탠스. 비판이 있더라도 건설적 대안 제시. | "AI는 위험하다/과대평가다/지능이 아니다" 등 회의론·경고 프레임이 본인의 자연스러운 포지션. 비판만 있고 대안 없음. |
